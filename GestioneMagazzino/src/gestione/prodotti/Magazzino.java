...
public class Magazzino{
	...
	public boolean aggiungiProdotto(Prodotto p) throws Exception {
		...
		if(p==null) {
		        	...
		        	/*
						if(scaffale[i][j].getMarca()==p.getMarca() && scaffale[i][j].getModello()==p.getModello() && 
			        	   scaffale[i][j].getQuantita()==p.getQuantita() && scaffale[i][j].getPrezzo()==p.getPrezzo()) {
			        		...
			        		System.out.println("La quantit� del prodotto � stata aggiornata a "+ scaffale[i][j].getQuantita()+" perch� il prodotto che volevi aggiungere era gi� stato inserito.");
			        		return true;
			        	}
		        	*/
		    ...
		    throw new Exception("Il prodotto non � stato inserito perch� lo scaffale � pieno.");
		}
	...
	public void modificaProdotto(Prodotto p)throws Exception {
		...
		if(p==null) {
			...
			case 3:					//Quantita
				int qu=0;
				System.out.println("Ora stai modificando la quantit�.");
				System.out.println("Inserisci la nuova quantit�:");
				qu=s.nextInt();
				p.setQuantita(qu);
				break;
			case 4:					//Prezzo
